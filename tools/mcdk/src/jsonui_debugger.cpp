...
namespace mcdk::jsonui_debugger {
    ...
    static std::string helpText(std::string_view command = {}) {
...
- Avoid raw recursive JSON UI APIs on large roots.)";
    ...
    bool commandHasFlag(std::string_view command, std::string_view flag) {
        std::size_t pos = 0;
    ...
    std::optional<std::string> commandOptionValue(std::string_view command, std::string_view optionName) {
    ...
    static std::string commandNameAfterHelp(std::string_view command) {
    ...
    nlohmann::json buildLocalHelpJson(std::string_view command) {
        const auto detail = commandNameAfterHelp(command);
        return nlohmann::json{{"ok", true}, {"cmd", "/help"}, {"data", {{"text", helpText(detail)}}}};
    }

    static std::string jsonEscapeForPySingleQuotedString(std::string_view value) {
        std::string out;
    ...
    std::string buildPythonCode(std::string_view command) {
...
try:
    ...
    static std::optional<std::string> extractFirstJsonText(std::string_view text) {
    ...
    static nlohmann::json parseJsonMaybeNestedString(std::string_view text) {
    ...
    static std::optional<std::string> extractJsonStringLiteralAtStart(std::string_view text) {
        std::size_t start = 0;
    ...
    static nlohmann::json parseExecuteCodeText(std::string_view text) {
    ...
    nlohmann::json parseFirstJsonFromDirtyText(std::string_view text) {
    ...
    static std::string htmlEscape(std::string_view value, bool escapeQuote = false) {
        std::string out;
        out.reserve(value.size() + 16);
    ...
    static std::string htmlTagForType(std::string_view typeName) {
        if (typeName == "Button") {
            return "button";
        }
    ...
    static double jsonNumberOr(const nlohmann::json& object, std::string_view key, double fallback) {
    ...
    static std::string jsonStringOr(const nlohmann::json& object, std::string_view key, std::string fallback) {
        if (!object.is_object() || !object.contains(std::string(key))) {
            return fallback;
        }
        const auto& value = object[std::string(key)];
    ...
    static bool jsonBoolOr(const nlohmann::json& object, std::string_view key, bool fallback) {
        if (!object.is_object() || !object.contains(std::string(key))) {
            return fallback;
        }
        const auto& value = object[std::string(key)];
    ...
    static std::string layoutExprToCss(const nlohmann::json& value, std::string_view axis) {
        if (!value.is_object()) {
            return {};
        }
    ...
    static void addStyle(std::vector<std::pair<std::string, std::string>>& style, std::string key, std::string value) {
        if (!value.empty()) {
            style.emplace_back(std::move(key), std::move(value));
        }
    }

    static std::string joinStyle(const std::vector<std::pair<std::string, std::string>>& style) {
        std::ostringstream out;
        bool               first = true;
    ...
    static bool jsonArrayHasNumbers(const nlohmann::json& array, std::size_t count) {
    ...
    static std::string jsonNumberPairAttr(const nlohmann::json& array) {
        std::ostringstream out;
        out << jsonScalarToString(array[0]) << ',' << jsonScalarToString(array[1]);
        return out.str();
    }

    static std::string nodeHtmlAttrs(const nlohmann::json& node) {
        ...
        if (node.contains("computed") && node["computed"].is_object()) {
            ...
            if (jsonArrayHasNumbers(global, 2)) {
                attrs.push_back(
                    "data-runtime-global-position=\"" + htmlEscape(jsonNumberPairAttr(global), true) + "\""
                );
            }
        }
        ...
        if (layout.is_object()) {
            if (layout.contains("anchor_from")) {
                attrs.push_back(
                    "data-anchor-from=\"" + htmlEscape(jsonScalarToString(layout["anchor_from"]), true) + "\""
                );
            }
        ...
        if (layout.is_object()) {
            addStyle(
                style,
                "width",
                layoutExprToCss(layout.contains("size_x") ? layout["size_x"] : nlohmann::json(), "x")
            );
            addStyle(
                style,
                "height",
                layoutExprToCss(layout.contains("size_y") ? layout["size_y"] : nlohmann::json(), "y")
            );
            addStyle(
                style,
                "left",
    ...
    static std::string nodeText(const nlohmann::json& node) {
    ...
    static std::string treeToHtmlPseudo(const nlohmann::json& node, int indent = 0) {
    ...
    static double jsonArrayNumberOr(const nlohmann::json& array, std::size_t index, double fallback) {
    ...
    static const char* svgColorForType(std::string_view typeName) {
        if (typeName == "Button") {
            return "#2f80ed";
        }
        if (typeName == "Label" || typeName == "RichText") {
    ...
    static void collectSvgNodes(const nlohmann::json& node, std::vector<SvgNodeRef>& out, int depth = 0) {
        if (!node.is_object() || node.contains("error")) {
            return;
        }
    ...
    static std::string renderLabelForNode(const nlohmann::json& node, std::string_view labelMode) {
        if (labelMode == "none") {
            return {};
        }
        if (labelMode == "type") {
    ...
    static std::string svgTitleForNode(const nlohmann::json& node) {
        std::ostringstream out;
    ...
    static std::string treeToSvgDiagram(const nlohmann::json& data) {
        if (!data.is_object() || !data.contains("tree") || !data["tree"].is_object()) {
            return {};
        }
        const auto& root = data["tree"];
        const auto& computed =
            root.contains("computed") && root["computed"].is_object() ? root["computed"] : nlohmann::json();
        const auto&  size       = computed.contains("size") ? computed["size"] : nlohmann::json();
        ...
        svg << "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 " << width << ' ' << viewHeight << "\" width=\""
            << width << "\" height=\"" << viewHeight << "\">\n";
        svg << "  <rect x=\"0\" y=\"0\" width=\"" << width << "\" height=\"" << viewHeight << "\" fill=\"#ffffff\"/>\n";
        ...
        for (std::size_t i = 0; i < nodes.size(); ++i) {
            ...
            const auto& comp =
                node.contains("computed") && node["computed"].is_object() ? node["computed"] : nlohmann::json();
            ...
            if (w <= 0.5 || h <= 0.5) {
                ...
                svg << "    <g opacity=\"" << strokeOpacity << "\"><title>" << htmlEscape(svgTitleForNode(node))
                    << "</title><line x1=\"" << cx - 3 << "\" y1=\"" << cy << "\" x2=\"" << cx + 3 << "\" y2=\"" << cy
                    << "\" stroke=\"" << color << "\" stroke-width=\"1\"/><line x1=\"" << cx << "\" y1=\"" << cy - 3
                    << "\" x2=\"" << cx << "\" y2=\"" << cy + 3 << "\" stroke=\"" << color
                    << "\" stroke-width=\"1\"/></g>\n";
                continue;
            }
            ...
            svg << "    <rect x=\"" << x << "\" y=\"" << y << "\" width=\"" << w << "\" height=\"" << h << "\" fill=\""
                << color << "\" fill-opacity=\"" << opacity << "\" stroke=\"" << color << "\" stroke-opacity=\""
                << strokeOpacity << "\" stroke-width=\"" << (i == 0 ? 1.4 : 0.8) << "\"><title>"
                << htmlEscape(svgTitleForNode(node)) << "</title></rect>\n";

            const auto label = renderLabelForNode(node, labelMode);
            if (!label.empty() && w >= 24.0 && h >= 10.0) {
                const double labelWidth =
                    std::min(w - 2.0, 8.0 + static_cast<double>(std::min<std::size_t>(label.size(), 42)) * 4.8);
                ...
                svg << "    <text x=\"" << x + 4 << "\" y=\"" << y + 10
                    ...
                    << htmlEscape(svgTitleForNode(node)) << "</title>" << htmlEscape(label.substr(0, 42))
                    << "</text>\n";
            }
        }
        ...
        if (showLegend) {
            ...
            svg << "    <text y=\"0\">Panel gray, Button blue, Label green, Image purple, Stack/Grid orange, "
                   "Custom/Factory red</text>\n";
            svg << "    <text y=\"11\">Generated from Minecraft runtime layout data; not a screenshot and not source "
                   "JSON UI.</text>\n";
            svg << "  </g>\n";
        }
        svg << "</svg>";
