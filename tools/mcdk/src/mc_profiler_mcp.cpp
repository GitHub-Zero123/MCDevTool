...
namespace {
    ...
    Json buildHelp(std::string_view topic) {
        ...
        if (topic.empty()) {
            ...
            if (topic == "native.cpu") {
                ...
                data["note"] = "Python profiling uses game IPC and does not initialize the Native component.";
            }
    ...
    Json buildGuide(std::string_view name) {
        ...
        if (name == "lag") {
            ...
            steps.push_back(Json{{"op", "/query"}, {"args", {{"view", "hotspots"}, {"limit", 20}}}});
        } else if (name == "memory-growth") {
            steps.push_back(Json{{"op", "/start"}, {"args", {{"kind", "python.memory"}, {"duration_seconds", 20}}}});
            steps.push_back(Json{{"op", "/query"}, {"args", {{"view", "growth"}, {"limit", 20}}}});
        } else if (name == "native-hotspot") {
            steps.push_back(Json{{"op", "/doctor"}, {"args", {{"kind", "native.cpu"}}}});
