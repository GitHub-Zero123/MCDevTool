...
// 本层不认识插件系统（mcdevtool 是 mcdk_runtime 的上游），所以只提供钩子，
    ...
    private:
        ...
        std::function<void(std::size_t, bool, unsigned short)> mClientCountChanged;
        void notifyClientCountChanged(std::size_t count, bool connected, unsigned short port) const;
