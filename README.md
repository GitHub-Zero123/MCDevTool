...
- `mc_input`：单工具命令驱动游戏窗口的键鼠输入，一次调用可完成点击、长按、拖拽、滚轮、视角转动、文本输入与等待组成的整串操作；`/timeline` 以绝对时刻编排，按键与鼠标的按住可以相互重叠（按住 W 前进的同时跳跃、点击）。先用 `/help` 查看用法，`/state` 查询窗口与指针状态。成功仅代表输入已投递，需配合截图或日志确认生效。
