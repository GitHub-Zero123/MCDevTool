...
7. `mc_input`：仅在测试入口无法覆盖且用户明确需要交互时使用；先 `/help` 了解步骤与参数，必要时 `/state` 确认指针是否被游戏独占；一次调用用 `/run` 提交整串操作，需要按住相互重叠（边前进边跳、边按住左键边转视角）时用 `/timeline` 以 `at_ms` + `hold_ms` 编排并先 `dry_run` 检查 `data.timeline.plan`；其成功只代表输入已投递，仍需截图或日志确认结果；使用 `leave_held` 后必须以 `/release-all` 收尾。聊天与指令走真实玩家路径：`key t` → `wait` → `text "/..."` → `key enter`，Mod 监听到的聊天 / 指令事件与真人一致；
