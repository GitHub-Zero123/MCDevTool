...
HTML 伪表达只用于帮助 AI 理解 Minecraft 当前运行时的 UI 布局。它来自游戏实时计算后的节点树、可见性、锚点、尺寸和坐标等数据，不是 JSON UI 源文件的反编译结果，也不代表浏览器可以精确复刻 Minecraft 的布局算法。
...
- 看运行时计算后的尺寸、位置和锚点关系。
- 辅助判断 JSON UI 修改方向。
- 不用它反推完整源码，不把它当作真实 HTML/CSS 渲染结果。
