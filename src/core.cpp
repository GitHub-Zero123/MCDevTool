// mcdk_core: 启动器实现
//
// 本文件由 tools/mcdk/main.cpp 搬迁而来，逻辑保持逐字一致（零行为变更）。
// 对外通过 mcdk::core::Launcher::run / loadConfigFromFile / createDefaultConfig / printLogo 暴露。
//
// By Zero123
#include <algorithm>
#include <sstream>
...
// 进程级注入式 logger。run() 进入时若 options.logger 非空则设置，退出时恢复为 nullptr。
// 设置后所有线程（含 watcher/pipe reader 子线程）通过 printColoredAtomic 的输出都会转发到该回调；
// 为 nullptr 时回退到内置 Win32 彩色输出（等价于原 mcdk.exe 行为）。
static mcdk::ConsoleOutputCallback g_injectedLogger = nullptr;

// 线程安全彩色输出
static void printColoredAtomic(const std::string& msg, ConsoleColor color) {
    // 优先走注入式 logger（业务自定义日志体系）
    if (g_injectedLogger) {
        std::lock_guard<std::mutex> lk(g_consoleMutex);
        g_injectedLogger(msg, color);
        return;
    }
...
nlohmann::json mcdk::core::loadConfigFromFile(const std::filesystem::path& configPath) {
    // 复用 modules/config.hpp 的 userParseConfig 语义，但允许指定路径。
    // 为保持与原行为完全一致（无参时读取当前目录 .mcdev.json），这里直接调用 userParseConfig，
    // 它内部使用 std::filesystem::current_path() / ".mcdev.json"。
    (void)configPath; // 路径参数保留以备未来扩展；当前沿用 userParseConfig 默认行为
    return mcdk::userParseConfig();
}

nlohmann::json mcdk::core::createDefaultConfig() {
    return mcdk::createDefaultConfig();
}

void mcdk::core::printLogo(bool pluginEnv) {
    printStartupLogo(pluginEnv);
}

int mcdk::core::Launcher::run(mcdk::core::LaunchOptions options) {
    // 决定 pluginEnv：options.pluginEnv == -1 时沿用环境变量
    bool pluginEnv = (options.pluginEnv == -1) ? mcdk::getEnvIsPluginEnv() : (options.pluginEnv == 1);

    // 设置注入式 logger（若提供），run() 返回前恢复
    mcdk::ConsoleOutputCallback prevLogger = g_injectedLogger;
    if (options.logger) {
        g_injectedLogger = options.logger;
    }
    ...
    // RAII 恢复
    struct LoggerGuard {
        mcdk::ConsoleOutputCallback& slot;
        mcdk::ConsoleOutputCallback   prev;
